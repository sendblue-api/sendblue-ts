...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['SENDBLUE_API_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: SendblueAPI) => {
  ...
  apiSecret: 'My API Secret',
    ...
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'SENDBLUE_API_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  apiSecret: 'My API Secret',
    ...
    afterEach(() => {
      process.env['SENDBLUE_API_BASE_URL'] = undefined;
    });
