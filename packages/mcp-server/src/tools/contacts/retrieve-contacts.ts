...
export const handler = async (client: SendblueAPI, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.contacts.retrieve(phone_number)));
  } catch (error) {
